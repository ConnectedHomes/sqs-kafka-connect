...
public class SQSStreamSourceTask extends SourceTask {
    ...
    public void start(Map<String, String> props) {
        queue = props.get(SQSStreamSourceConnector.SOURCE_SQS_QUEUE);
        if (queue == null)
            throw new ConnectException("SQSStreamSourceTask config missing queue setting");
        topic = props.get(SQSStreamSourceConnector.DESTINATION_KAFKA_TOPIC);
        if (topic == null)
            throw new ConnectException("SQSStreamSourceTask config missing topic setting");
        awsKey = Optional.ofNullable(props.get(SQSStreamSourceConnector.AWS_KEY));
        awsSecret = Optional.ofNullable(props.get(SQSStreamSourceConnector.AWS_SECRET));
    }
